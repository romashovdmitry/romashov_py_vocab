...
@app.route('/modificate/<id>', methods=['POST', 'GET'])
def modificateWordInTable(id):
    '''URL for page, Where user modificate word and/or definition'''
    try:
        word = db.session.query(whole_vocab).filter_by(id_in_whole=id).first().word_in_whole
        definition = db.session.query(whole_vocab).filter_by(id_in_whole=id).first().definition_of_word
